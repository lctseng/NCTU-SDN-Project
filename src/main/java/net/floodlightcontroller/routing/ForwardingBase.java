...
public abstract class ForwardingBase
    ...
    public boolean pushRoute(Route route, OFMatch match,
        ...
        return pushRoute(route,match,wildcard_hints,pi,pinSwitch,cookie,cntx,reqeustFlowRemovedNotifn,doFlush,flowModCommand,false,0);
        
    }
    
    // TODO:SDN:Function Make Cookie
    private long makeCookieFromPriority(int app,short outPort ,int priority){
        int sdn_cookie = ( priority << 16 ) | outPort;
        return AppCookie.makeCookie(app, sdn_cookie);
    }
    ...
    public boolean pushRoute(Route route, OFMatch match,
        ...
        for (int indx = switchPortList.size()-1; indx > 0; indx -= 2) {
            ...
            // TODO:SDN Change cookie with outport
            if(changeCookie){
                cookie = makeCookieFromPriority(AppCookie.extractApp(cookie),outPort,priority);
                fm.setCookie(cookie);
            }
            // TODO:SDN Register LinkMap
            if(routingEngine != null){
                if(switchPortList.size()-1 != indx){
                    OFMatch m = new OFMatch();
                    m.setInputPort(inPort);
                    m.setDataLayerDestination(match.getDataLayerDestination());
                    m.setDataLayerSource(match.getDataLayerSource());
                    m.setDataLayerVirtualLan(match.getDataLayerVirtualLan());
    ...
    // TODO:SDN Clean Lower Priority
    private void clearPriorityFlowEntries(long switchDPID,short outPort,int priority){
        
        long cookie = makeCookieFromPriority(FORWARDING_APP_ID,outPort ,priority);
        //System.out.printf("$$$ Cleaning Flow entry..\n");
        OFMatch match = new OFMatch();
        match.setWildcards(OFMatch.OFPFW_DL_DST | OFMatch.OFPFW_DL_SRC | OFMatch.OFPFW_DL_VLAN | OFMatch.OFPFW_IN_PORT | OFMatch.OFPFW_DL_TYPE | OFMatch.OFPFW_DL_VLAN_PCP | OFMatch.OFPFW_NW_DST_ALL | OFMatch.OFPFW_NW_PROTO | OFMatch.OFPFW_NW_SRC_ALL | OFMatch.OFPFW_NW_TOS | OFMatch.OFPFW_TP_DST | OFMatch.OFPFW_TP_SRC) ;
        IOFSwitch sw = floodlightProvider.getSwitch(switchDPID);
        //System.out.printf("$$$ Work on SW %s , Priority %d , cookie:%s \n", sw.getId(),priority,cookie);
        // send flow_mod delete
        OFFlowMod fm = (OFFlowMod) floodlightProvider
           .getOFMessageFactory().getMessage(OFType.FLOW_MOD);
        
        fm.setCommand(OFFlowMod.OFPFC_DELETE); 
        fm.setMatch(wildcard(match, sw, match.getWildcards()));
        fm.setCookie(cookie);
